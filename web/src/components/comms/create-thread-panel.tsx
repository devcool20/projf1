...
export function CreateThreadPanel({ profile, onSuccess }: Props) {
  ...
  return (
    <SharedLayout itemId="create-thread-panel">
      <motion.div
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
        className="comms-light-theme dashboard-panel p-5"
      >
        <div className="flex items-center gap-3 border-b border-primary/20 pb-4 mb-5">
          ...
          >
            {profile?.avatar_url ? (
              <ImageWithLoader src={profile.avatar_url} alt={profile.username} className="h-full w-full object-cover" containerClass="h-full w-full" />
            ) : (
              <span className="text-secondary">{profile?.username?.[0]?.toUpperCase() || "P"}</span>
            )}
          </div>
          <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <AnimatePresence>
            {previewUrl && (
              ...
              >
                <ImageWithLoader src={previewUrl} alt="Preview" className="h-full w-full object-cover" containerClass="h-full w-full" />
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  type="button"
                  onClick={() => {
